...
const LiquidCursor = () => {
  ...
  // Separate springs for the outer ring (avoids calling hooks inside render)
  const outerXSpring = useSpring(cursorX, { damping: 30, stiffness: 200 });
  const outerYSpring = useSpring(cursorY, { damping: 30, stiffness: 200 });
  ...
  useEffect(() => {
    ...
    const handleHover = (e: MouseEvent) =>
      setIsHovering(!!(e.target as HTMLElement).closest('button, a, [data-hover]'));
  ...
  return (
    <div className="hidden md:block pointer-events-none fixed inset-0 z-[9999]">
      {trail.map((point, i) => (
        <motion.div
          ...
          style={{
            left: point.x - 4,
            top: point.y - 4,
            width: Math.max(8 - i * 0.5, 1),
            height: Math.max(8 - i * 0.5, 1),
          }}
        />
      ))}
      ...
      {/* Outer ring */}
      <motion.div
        className="fixed border-2 border-cyan-400/50 rounded-full"
        style={{
          x: outerXSpring,
          y: outerYSpring,
          translateX: '-50%',
...
// ─── Text Scramble ────────────────────────────────────────────────────────────
type ScrambleTextProps = { text: string; className?: string };
...
// ─── 3D Tilt Card ─────────────────────────────────────────────────────────────
type Card3DProps = { children: React.ReactNode; className?: string };
...
const Card3D = ({ children, className }: Card3DProps) => {
  ...
  return (
    ...
    >
      {children}
      <div
        className="absolute inset-0 rounded-2xl pointer-events-none opacity-0 group-hover:opacity-20 transition-opacity duration-300"
        style={{
...
const MagneticBtn = ({ children, className, onClick, href }: MagneticBtnProps) => {
  const ref = useRef<HTMLDivElement>(null);
...
const Particles = ({ isDark }: { isDark: boolean }) => {
  // Fixed positions to avoid hydration mismatch on re-renders
  const particles = Array.from({ length: 25 }, (_, i) => ({
    left: ((i * 37 + 11) % 100).toFixed(1),
    top: ((i * 53 + 7) % 100).toFixed(1),
    width: (((i * 17) % 6) + 2).toFixed(1),
    duration: (((i * 7) % 15) + 10).toFixed(1),
    xOffset: ((i * 13) % 30) - 15,
  }));
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
      {particles.map((p, i) => (
        <motion.div
          key={i}
          className={`absolute rounded-full ${isDark ? 'bg-cyan-400/20' : 'bg-cyan-600/10'}`}
          style={{ left: `${p.left}%`, top: `${p.top}%`, width: p.width, height: p.width }}
          animate={{ y: [0, -40, 0], x: [0, p.xOffset, 0], opacity: [0.2, 0.5, 0.2] }}
          transition={{ duration: Number(p.duration), repeat: Infinity, ease: 'easeInOut' }}
        />
      ))}
    </div>
  );
...
const blogPosts = [
  {
    title:
      'Mastering Cross-Platform Swift Development: How to Set Up Skip, Manage Dependencies, and Write Smarter Code',
    date: 'July 30, 2025',
    excerpt:
      'Unlock powerful insights every Swift developer should know for building truly native iOS and Android apps—all from one codebase using Skip.',
...
const linkedInPosts = [
  {
    ...
    date: 'July 29, 2025',
    excerpt: 'Sharing insights on cross-platform development using Swift and SwiftUI.',
    url: 'https://www.linkedin.com/in/anupamthackar',
    tags: ['SwiftLang', 'MobileDevelopment'],
  },
  {
  ...
  },
  {
    ...
    url: 'https://www.linkedin.com/in/anupamthackar',
    tags: ['iOSDeveloper', 'AI', 'Swift'],
  },
  {
    title: 'My E-commerce UI App Built Entirely in SwiftUI',
    excerpt: 'Personal project to sharpen my SwiftUI skills.',
    url: 'https://www.linkedin.com/in/anupamthackar',
    tags: ['SwiftUI', 'UIDesign'],
  },
];
...
// ─── Project Card (shared between anchor + div variants) ──────────────────────
...
const ProjectCardContent = ({ project, index, isDark, t }: ProjectCardContentProps) => (
  <>
    {/* Top gradient bar */}
    ...
    <div className="p-6 flex flex-col flex-1">
      ...
      <div className="flex items-start justify-between mb-4">
        <div
          className={`p-3 rounded-xl bg-gradient-to-br ${project.color} bg-opacity-20 text-white group-hover:scale-110 transition-transform shadow-lg`}
        >
          {project.icon}
        </div>
        <span className={`text-[10px] font-mono ${t.faint} uppercase tracking-wider`}>
          Project {String(index + 1).padStart(2, '0')}
        </span>
      </div>

      {/* Title */}
      <h3 className={`text-xl font-bold ${t.text} mb-1 group-hover:text-cyan-400 transition-colors`}>
        {project.title}
      </h3>
      ...
      <div className="flex flex-wrap gap-1.5 mb-4">
        {project.tags.map((tag, j) => (
          <span
            key={j}
            className={`px-2 py-0.5 rounded-full text-[10px] font-mono bg-gradient-to-r ${project.color} bg-opacity-10 text-white border border-cyan-500/20`}
          >
            {tag}
          </span>
        ))}
      ...
      <ul className="space-y-2 flex-1">
        {project.features.map((feature, j) => (
          ...
          >
            <span
              className={`w-1.5 h-1.5 rounded-full bg-gradient-to-r ${project.color} mt-1.5 flex-shrink-0 shadow-[0_0_4px_rgba(6,182,212,0.6)]`}
            />
            {feature}
          </motion.li>
        ))}
...
// ─── Theme token helper (extracted to reuse type) ─────────────────────────────
const useThemeTokens = (isDark: boolean) => ({
...
function App() {
  ...
  // Shared card class for project cards
  const projectCardClass = `${t.card} border backdrop-blur-sm rounded-2xl overflow-hidden ${t.cardHover} transition-all h-full flex flex-col relative`;

  return (
    <div className={`relative overflow-x-hidden ${t.bg} transition-colors duration-500`}>
      {/* Conditional heavy UI — desktop only */}
      ...
      <motion.button
        ...
        className={`fixed top-6 right-6 z-[9998] w-12 h-12 rounded-full backdrop-blur-xl border flex items-center justify-center shadow-xl ${
      ...
      >
        <AnimatePresence mode="wait">
          {isDark ? (
            <motion.div key="m" initial={{ scale: 0, rotate: -180 }} animate={{ scale: 1, rotate: 0 }} exit={{ scale: 0 }}>
              <Moon className="w-5 h-5 text-cyan-400" />
            </motion.div>
          ) : (
            <motion.div key="s" initial={{ scale: 0, rotate: 180 }} animate={{ scale: 1, rotate: 0 }} exit={{ scale: 0 }}>
  ...
  <span className="hidden sm:inline">Resume</span>
      ...
      >
        <div className="relative">
          <div className={`w-1 h-56 rounded-full ${isDark ? 'bg-slate-800' : 'bg-gray-300'}`}>
            <motion.div
              className="w-full bg-gradient-to-b from-cyan-400 to-blue-500 rounded-full shadow-[0_0_15px_rgba(34,211,238,0.6)]"
          ...
          <div className="absolute left-5 top-0 flex flex-col h-full justify-between py-0.5">
            {sections.map((s) => (
              <motion.button
                ...
                className={`text-[9px] font-mono uppercase tracking-wider transition-all whitespace-nowrap ${
                  activeSection === s.id
                    ? 'text-cyan-400 translate-x-1 font-bold'
                    : isDark
                ...
                whileHover={{ x: 5 }}
              >
                {activeSection === s.id && <span className="mr-1 text-cyan-400">›</span>}
                {s.label}
              </motion.button>
            ))}
          </div>
      ...
      <section id="hero" className="min-h-screen flex items-center justify-center px-6 lg:pl-28 relative">
        ...
        <div
          ...
          style={{
            backgroundImage: `linear-gradient(to bottom, transparent, rgba(6,182,212,0.1)),
        ...
        <div className="text-center relative z-10 max-w-4xl mx-auto">
          ...
          >
            <motion.div
              className="w-28 h-28 sm:w-36 sm:h-36 md:w-44 md:h-44 rounded-full bg-gradient-to-br from-cyan-400 to-blue-600 p-1"
              animate={{
                boxShadow: [
            ...
            {/* Orbit rings */}
            <motion.div
              className="absolute inset-[-12px] rounded-full border-2 border-cyan-400/30"
              animate={{ rotate: 360 }}
            ...
            <motion.div
              className="absolute inset-[-24px] rounded-full border border-cyan-400/15"
              animate={{ rotate: -360 }}
              transition={{ duration: 15, repeat: Infinity, ease: 'linear' }}
            />
          </motion.div>

          {/* H1 — semantic heading for SEO */}
          <motion.h1
          ...
          {/* Scramble badge */}
          <motion.div
            className="mb-5 inline-block px-5 py-2 rounded-full bg-cyan-500/10 border border-cyan-500/30 backdrop-blur-sm"
          ...
          <motion.p
            className={`text-base md:text-lg ${t.muted} max-w-2xl mx-auto mb-8 leading-relaxed`}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
  ...
  Delivered AI-powered, real-time features across fintech, banking & travel.
          ...
          >
            <MagneticBtn
              ...
              className="px-7 py-3 rounded-full bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-semibold shadow-lg shadow-cyan-500/25 hover:shadow-cyan-500/40 transition-shadow"
            >
              Get In Touch
            </MagneticBtn>
            <MagneticBtn
              onClick={() => scrollTo('projects')}
              className={`px-7 py-3 rounded-full border font-semibold transition-colors ${
                isDark
                  ? 'border-slate-600 text-slate-300 hover:border-cyan-500 hover:text-cyan-400'
                  : 'border-gray-300 text-slate-600 hover:border-cyan-500 hover:text-cyan-500'
              }`}
            >
              View Projects
            </MagneticBtn>
      ...
      <section id="experience" className={`min-h-screen flex items-center py-20 px-6 lg:pl-28 ${t.section}`}>
        <div className="max-w-5xl mx-auto w-full">
          ...
          <Card3D className="group">
            <motion.div
              className={`${t.card} border backdrop-blur-sm rounded-2xl p-7 md:p-10 ${t.cardHover} transition-all relative overflow-hidden`}
            ...
            >
              <div className="grid md:grid-cols-3 gap-6">
                <div className="md:text-right md:pr-6 md:border-r border-cyan-500/30">
                  <div className="flex items-center md:justify-end gap-2 text-cyan-400 mb-2">
                  ...
                  <ul className="space-y-3">
                    {[
                      ...
                      <motion.li
                        ...
                        viewport={{ once: true }}
                        className={`flex items-start gap-3 ${t.muted}`}
                      >
                        <span className="w-2 h-2 rounded-full bg-gradient-to-r from-cyan-400 to-blue-500 mt-2 flex-shrink-0 shadow-[0_0_6px_rgba(6,182,212,0.8)]" />
                        {item}
                      </motion.li>
                    ))}
                  </ul>
                ...
                <Card3D className="group h-full">
                  <div className={`${t.card} border backdrop-blur-sm rounded-2xl p-6 ${t.cardHover} transition-all h-full`}>
                    <div className="flex items-center gap-3 mb-4">
                      <div className="p-2.5 rounded-xl bg-gradient-to-br from-cyan-500/20 to-blue-500/20 text-cyan-400 group-hover:scale-110 transition-transform">
                        {g.icon}
                      </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {g.items.map((s, j) => (
                        <motion.span
                          ...
                          } border hover:bg-cyan-500/10 hover:border-cyan-500/30 hover:text-cyan-400 transition-all cursor-default`}
      ...
      <section id="projects" className={`py-20 px-6 lg:pl-28 ${t.section}`}>
        <div className="max-w-6xl mx-auto w-full">
          ...
          <motion.p
            className={`text-center ${t.muted} mb-12 max-w-2xl mx-auto`}
            initial={{ opacity: 0 }}
            whileInView={{ opacity: 1 }}
            viewport={{ once: true }}
          >
            ...
            {projects.map((project, i) => (
              <Card3D key={i} className="group h-full">
                <motion.a
                  ...
                  aria-label={`View ${project.title} on ${project.url.includes('github') ? 'GitHub' : 'App Store'}`}
                  ...
                  whileHover={{ y: -8 }}
                >
                  <ProjectCardContent project={project} index={i} isDark={isDark} t={t} />
                </motion.a>
              </Card3D>
            ))}
          </div>
          ...
          >
            {[
              ...
              <motion.div key={i} variants={i === 0 ? revealFromLeft : revealFromRight}>
                <Card3D className="group h-full">
                  <div
                    className={`${t.card} border backdrop-blur-sm rounded-2xl p-6 ${t.cardHover} transition-all h-full`}
                  >
                    <div className="flex items-center gap-2 text-cyan-400 mb-3">
                      <Calendar className="w-4 h-4" />
                      <span className="text-sm font-medium">{edu.period}</span>
                    </div>
                    <h3 className={`text-xl font-bold ${t.text} mb-2 group-hover:text-cyan-400 transition-colors`}>
                      {edu.title}
                    </h3>
                    <p className={t.muted}>{edu.desc}</p>
                    {edu.extra && (
                      <p className="text-cyan-400/80 text-sm font-medium mt-2">{edu.extra}</p>
                    )}
                  </div>
                </Card3D>
              </motion.div>
              ...
              <motion.div key={i} variants={staggerItem}>
                <Card3D className="group">
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className={`block ${t.card} border backdrop-blur-sm rounded-2xl p-6 ${t.cardHover} transition-all`}
                  >
                    ...
                    <h4 className={`text-lg font-bold ${t.text} mb-2 group-hover:text-cyan-400 transition-colors`}>
                      {p.title}
                    </h4>
                    <p className={`${t.muted} text-sm mb-3`}>{p.excerpt}</p>
                    <div className="flex flex-wrap gap-2">
                      {p.tags.map((tag, j) => (
                        <span key={j} className="px-2 py-1 rounded-full bg-cyan-500/10 text-cyan-400 text-xs">
                          {tag}
                        </span>
                      ))}
                    </div>
                ...
                <motion.div key={i} variants={staggerItem}>
                  <Card3D className="group h-full">
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className={`block ${t.card} border backdrop-blur-sm rounded-xl p-5 ${t.cardHover} transition-all h-full`}
                    >
                      <div className="flex items-center justify-between mb-2">
                        <Linkedin className="w-4 h-4 text-[#0A66C2]" />
                        <ExternalLink className={`w-3 h-3 ${t.faint} group-hover:text-cyan-400 transition-colors`} />
                      </div>
                      <h4 className={`text-base font-bold ${t.text} mb-1 group-hover:text-cyan-400 transition-colors`}>
                        {p.title}
                      </h4>
                        ...
                        {p.tags.slice(0, 3).map((tag, j) => (
                          <span
                            ...
                            className={`px-2 py-0.5 rounded-full text-[10px] ${
                              isDark ? 'bg-slate-700/50 text-slate-400' : 'bg-gray-100 text-slate-500'
                            }`}
                          >
                            #{tag}
                          </span>
            ...
            <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-3">
              {certifications.map((c, i) => (
                <motion.div
                  key={i}
                  variants={staggerItem}
                  className={`${t.card} border backdrop-blur-sm rounded-xl p-4 ${t.cardHover} transition-all flex items-center gap-2`}
            ...
            <div className="space-y-3">
              {achievements.map((a, i) => (
                <motion.div
                  key={i}
                  variants={staggerItem}
                  className={`flex items-start gap-3 ${t.card} border backdrop-blur-sm rounded-xl p-4 ${t.cardHover} transition-all`}
                  whileHover={{ x: 5 }}
                >
                  <span className="w-2 h-2 rounded-full bg-gradient-to-r from-cyan-400 to-blue-500 mt-2 flex-shrink-0 shadow-[0_0_6px_rgba(6,182,212,0.8)]" />
                  <span className={t.muted}>{a}</span>
                </motion.div>
              ))}
            </div>
          </motion.div>
      ...
      <section id="contact" className={`min-h-screen flex items-center py-20 px-6 lg:pl-28 ${t.section}`}>
        <div className="max-w-4xl mx-auto w-full text-center">
          ...
          <motion.p
            className={`text-base ${t.muted} mb-10 max-w-xl mx-auto`}
            variants={revealVariants}
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true, margin: '-50px' }}
          ...
          >
            <motion.div variants={staggerItem}>
              <MagneticBtn
                ...
                className={`flex items-center gap-3 px-6 py-4 ${t.card} border backdrop-blur-sm rounded-xl ${t.cardHover} transition-all group`}
              >
                <Mail className="w-5 h-5 text-cyan-400 group-hover:scale-110 transition-transform" />
                <span className={t.muted}>anupamthackar@gmail.com</span>
              </MagneticBtn>
            </motion.div>
            ...
            <motion.div variants={staggerItem}>
              ...
              >
                ...
                <span className={t.muted}>+91 8962219884</span>
              </MagneticBtn>
            </motion.div>
          </motion.div>
          ...
          >
            {[
              ...
              <motion.div key={i} variants={staggerItem}>
                <MagneticBtn
                  href={social.href}
                  className={`w-14 h-14 flex items-center justify-center rounded-xl ${t.card} border backdrop-blur-sm ${
                    isDark ? 'text-slate-400' : 'text-slate-500'
                  } hover:text-cyan-400 ${t.cardHover} transition-all`}
                >
                  {social.icon}
                </MagneticBtn>
              </motion.div>
      ...
      <footer className={`py-6 px-6 border-t ${isDark ? 'border-slate-800' : 'border-gray-200'}`}>
        <div className="max-w-4xl mx-auto text-center">
          <p className={`text-sm ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>
            © {new Date().getFullYear()} Anupam Thackar. All rights reserved.
          </p>
        </div>
      </footer>
    </div>
